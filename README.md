...
But sometimes it can be desirable to plug Sentry to an abritrary process. Such process can be a binary built without Sentry support or a Bash script that runs every night on a server, for example.
...
- run `sentry-process COMMAND [ARGUMENTS]...` (wrap your process call like you would do with `sudo`)
...
- Sentry Process does **not** handle standard input (`stdin`).
